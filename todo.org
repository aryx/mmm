...
*** RELAX lpize applets/sandbox/ files
and try to share chunks instead of copy paste and useless coupling

*** RELAX mmm: use capabilities exec! and forkew! where do we call mmmc ?

*** WAITING try to complete mmm.nw with Claude?
finish lpize what can be lpized

*** try animated gif, solve Tkanim linking problem?

*** start remove use of class and objects
use Claude
...
*** add mmm -dump_html and start use deriving

*** add mmm -dump_http ?

*** add css parser (and osemgrep lib_parsing? Tok.t ?)

*** mmm: demos sboard?

*** -------------------------------------------------------

*** see chapter 1 of browser-engineering book and all the
advanced features as exercises

*** read mrubeck blog posts on robinson toy browser
look at dom, css style, etc. ideas for implemtation my own.

*** when render href anchor, where is the code that actually go to the link?
the hyperfuncs stuff?
